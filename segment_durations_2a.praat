...
# - !!! the script assumes the name for the sound files as for the textgrid
...
form start
	...
	comment Where is the folder with the sounds?
	#sentence snd_path /Volumes/KOALA/Documents/Dagestan_Stops/Andi_Zilo_2017/
	sentence snd_path /Volumes/KOALA/Documents/Dagestan_Stops/Rutul_Kina_2016/
	comment What is the name of the results file?
	word outputf dagstop_seg_vot5_Rutul.txt
	#word outputf dagstop_seg_vot5_Andi.txt
	comment Load same named sound files from sound directory as long sounds?
	comment Sound name suffix
	word suffix _m
...
...tab$, "tstart", tab$, "tend", tab$, "reltime", tab$, "pfst", tab$, "cvst", tab$, "modst", tab$, "voicingdur", tab$, "voiceperc", tab$, "syllrate", tab$, "f0FolVow", tab$, "intFolVow", tab$, "comment"
...
for i to nrst
	...
	str$=str$-".TextGrid"
	if load==1
		ls=Open long sound file: "'snd_path$''str$''suffix$'.wav"
	endif
	cdtier=0
	voicetier=0
	wordtier=0
	...
	for iii to nri
		...
		if (li$!="a" and li$!="i" and li$!="ai" and li$!="o" and li$!="u" and li$!="x" and li$!="i" and li$!="e" and li$!="ẽ" and li$!="r" and li$!="ɨ:" and li$!="ɨ" and li$!="ʃ" and li$!="ʃtʃ" and li$!="š" and li$!="z" and li$!="ž"
			...
			#F0 in following vowel
			select ls
			fvpart=Extract part: fvlist-0.5, fvliet+0.5, "yes"
			fvpart_pitch = To Pitch (ac): 0, 75, 15, "no", 0.03, 0.45, 0.01, 0.35, 0.14, 600
			f0FolVow=Get quantile: modst, modst+0.03, 0.5, "Hertz"
			Remove
			#Intensity in following vowel
			select fvpart
			fvpart_int=To Intensity: 100, 0, "yes"
			intFolVow=Get quantile: modst, modst+0.03, 0.5
			Remove
			select fvpart
			Remove
			#pause f0
			...
			...tab$, st, tab$, et, tab$, reltime, tab$, pfst, tab$, cv2st, tab$, modst, tab$, voicingdur, tab$, voiceperc, tab$, syllrate, tab$, f0FolVow, tab$, intFolVow, tab$, comm$
